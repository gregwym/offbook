...
- **transactions** — `id, account_id, category_id, amount, currency, description, description_clean, merchant_name, transaction_date, posted_date, source, external_id, plaid_transaction_id, categorization_method, categorization_rule_id, is_transfer, transfer_pair_id, notes, created_at, updated_at, deleted_at`
  - `source`: `'plaid' | 'csv' | 'pdf' | 'manual'`
  - `categorization_method`: `'manual'` (user picked) | `'rule'` (user `categorization_rules` matched at import) | `'plaid_default'` (auto-assigned from `plaid_category_map` at Plaid sync). Null when the row is uncategorized. Resolution order at insert: rule → plaid_default → uncategorized. Manual always wins on merge — see `service/plaid/transaction_mapping.go::MergePlaidUpdate`. Rule engine lives at `internal/service/categorization/engine.go`.
  - `categorization_rule_id`: nullable FK to `categorization_rules(id)`, `ON DELETE SET NULL`. Records which rule fired; pure provenance — the row's `category_id` is the source of truth.
...
- **categorization_rules** — `user_id, pattern, category_id, match_type ('contains'|'regex'|'exact'), priority, is_active`. User-scoped (no cross-user sharing). CRUD lives at `/api/v1/categorization-rules` via `service.CategorizationRuleService`; evaluation lives at `service/categorization.Categorize`. Plaid sync loads the user's compiled rules once per drain via `service.WithRuleRepo`; manual `transaction_service.Create` loads them lazily when no `CategoryID` is supplied.
