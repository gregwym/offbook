...
(Sonnet, `--permission-mode auto`) that ships **at most one PR** and exits.
...
Claude iOS app. Running locally fixes both: **auto mode** (`--permission-mode
auto`) lets a safety classifier approve/deny each tool call in-process, so
nothing ever waits on a human, and every iteration writes a plain log file you
...
- **Auto mode, not skip-permissions.** The iteration runs with
  `--permission-mode auto`: the Claude Code safety classifier gates every tool
  call in-process (owner decision, July 2026 — `--dangerously-skip-permissions`
